Review the "## Example" section in the specified lesson file and rework it so it clearly follows the steps from the "Practical guidance" section above it.

The example should use this format: a short opening paragraph describing the context, followed by a numbered list where each item corresponds to one step from the practical guidance. Use the step name as a bold label. See docs/part-1/lesson-2.md for a good reference.

Guidelines:

1. **One item per step** — Every step in the practical guidance gets its own numbered item in the example. Do not skip or merge steps.
